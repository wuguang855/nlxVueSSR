...
function getDeviceId(context) {
	return context.$store.state.cookies['DEVICEID'] || "unknownVisitor";
}
//获取DEVICEID
function getCookie(context,name) { 
 	return context.$store.state.cookies[name] || null;
} 
//获取系统类型
function getSysName() { 
 	return "TEA_B2C";
...
function setCookie(context,name, value) {
	context.$store.commit('SETCOOKIES', [name, value]);
	if(window){
		var Days = 30;
		var exp = new Date();
...
function delCookie(context,name){
	context.$store.commit('SETCOOKIES', [name, '']);
	if(window){
		var Days = -1;	
		var exp = new Date();
		exp.setTime(exp.getTime() + Days * 24 * 60 * 60 * 1000);
